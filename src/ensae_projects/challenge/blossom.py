"""
@file
@brief Source: https://github.com/koniiiik/edmonds-blossom/blob/master/blossom.py
...
class Blossom(object):
    ...
    def alter_tree(self, roots):
        ...
        if self.level == LEVEL_ODD:
            ...
            self.handle_tight_edges(roots)
        else:
            assert False, ("alter_tree called on blossom of level %d" %
                           self.level)

        for child in self.children:
            child.alter_tree(roots)

    def handle_tight_edges(self, roots):
        ...
        for e, v in self.outgoing_edges:
            ...
            if other_blossom.get_root() == self.get_root():
                self.shrink_with_peer(other_blossom, e, roots)
            else:
                self.augment_matching(other_blossom, e, roots)
    ...
    def shrink_with_peer(self, other, edge, roots):
        ...
        cycle = parent_chain1[
            ancestors[common_ancestor]::-1] + parent_chain2[:-1]
    ...
    def augment_matching(self, other_blossom, edge, roots):
        ...
        assert edge.selected == 0, ("trying to augment via an already "
                                    "selected edge")
        self.flip_root_path(edge, roots)
        other_blossom.flip_root_path(edge, roots)
        edge.toggle_selection()
        raise TreeStructureChanged("Augmented on edge %s" % edge)

    def flip_root_path(self, edge, roots):
        ...
        if self.level == LEVEL_EVEN:
            self.detach_children(roots)
            # Become a peer to the blossom on the other side of edge.
            self.detach_from_parent(edge, roots)
        else:
        ...
        if prev_parent is not None:
            ...
            prev_parent.flip_root_path(prev_parent_edge, roots)
    ...
    def detach_children(self, roots):
        ...
        for child in list(self.children):
            child.detach_from_parent(None, roots)

    def detach_from_parent(self, edge, roots):
